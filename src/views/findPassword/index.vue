  ...
  <div class="login clearfix">
    ...
    <div class="login-wrap">
      <el-row type="flex" justify="center">
        <el-form ref="loginForm" :model="findPasswordForm" status-icon label-width="90px">
          <h3>找回密码</h3>
          <hr>
          ...
          <el-form-item prop="password" label="设置新密码">
            <el-input v-model="findPasswordForm.password" show-password placeholder="请输入新密码"></el-input>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" style="margin-left: -10px;margin-top: 10px" icon @click="setNewPassword">确定
            </el-button>
          </el-form-item>
        </el-form>
      </el-row>
  ...
  methods: {
    send() {
      if (!this.findPasswordForm.email) {
        ...
        return;
      } else if (this.findPasswordForm.email != null) {
        var reg = /^[A-Za-z0-9\u4e00-\u9fa5]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/;
        if (!reg.test(this.findPasswordForm.email)) {
          this.$message.warning("请输入有效的邮箱！");
        } else {
          request.get('/readerCard/getFindPasswordCaptcha?email=' + this.findPasswordForm.email).then(res => {
            if (res.code == 0) {
    ...
    setNewPassword() {
      if (!this.findPasswordForm.email) {
        ...
        if (!reg.test(this.findPasswordForm.email)) {
          ...
          return;
        } else if (!this.findPasswordForm.password) {
          this.$message.warning("请输入密码！");
          return;
        } else if (this.findPasswordForm.password) {
          var regPas = /(?![A-Z]*$)(?![a-z]*$)(?![0-9]*$)(?![^a-zA-Z0-9]*$)/;
          if (!regPas.test(this.findPasswordForm.password)) {
            this.$message.warning("请输入由大写字母、小写字母、数字、特殊符号中的2种及以上类型密码");
            return;
          }
        }
        request.post("/readerCard/findPassword", {
